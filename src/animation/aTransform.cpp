...
ATransform ATransform::Inverse() const
{
	...
	ATransform inverse;
	inverse.m_rotation = this->m_rotation.Transpose();
...
vec3 ATransform::RotTrans(const vec3& vecToTransform) const
{
	...
	vec3 rotTrans;
	rotTrans = Translate(Rotate(vecToTransform));
...
vec3 ATransform::Rotate(const vec3& vecToTransform) const
{
	...
	vec3 rotate;
	rotate = this->m_rotation * vecToTransform;
...
vec3 ATransform::Translate(const vec3& vecToTransform) const
{
	...
	vec3 translate;
	translate = this->m_translation + vecToTransform;
...
ATransform operator * (const ATransform& H1, const ATransform& H2)
{
	...
	ATransform newTransform;
	newTransform.m_rotation = H1.m_rotation * H2.m_rotation;
	newTransform.m_translation = H1.m_translation + H1.m_rotation * H2.m_translation;
